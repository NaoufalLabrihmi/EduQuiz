...
export default function PDFViewer({ pdfUrl, courseId, onComplete, onClose, onQuizClick }: PDFViewerProps) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col items-center justify-center pt-10 pb-2 animate-fade-in-fast absolute top-0 left-0 z-20 pointer-events-none">
      ...
      <div className="fixed bottom-0 left-0 w-full flex justify-center z-30 pointer-events-none">
        <div className="pointer-events-auto bg-white/80 backdrop-blur-xl rounded-full shadow-xl border border-fuchsia-200/30 px-8 py-3 flex gap-4 items-center animate-fade-in-up transition-all duration-500">
          <button onClick={zoomOut} className="hover:bg-fuchsia-100/60 active:bg-fuchsia-200 focus:ring-2 focus:ring-fuchsia-400 transition-all duration-150 rounded-full p-2 text-blue-700 disabled:opacity-40 disabled:cursor-not-allowed" title="Zoom Out">
            <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M15 12H9" /></svg>
          </button>
          <button onClick={() => changePage(-1)} disabled={pageNumber <= 1 || loading} className="hover:bg-fuchsia-100/60 active:bg-fuchsia-200 focus:ring-2 focus:ring-fuchsia-400 transition-all duration-150 rounded-full p-2 text-blue-700 disabled:opacity-40 disabled:cursor-not-allowed" title="Previous">
            <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M15 18l-6-6 6-6" /></svg>
          </button>
          <span className="font-bold text-blue-700 text-base select-none px-2">
            {pageNumber} / {numPages || '-'}
          </span>
          <button onClick={() => changePage(1)} disabled={pageNumber >= (numPages || 1) || loading} className="hover:bg-fuchsia-100/60 active:bg-fuchsia-200 focus:ring-2 focus:ring-fuchsia-400 transition-all duration-150 rounded-full p-2 text-blue-700 disabled:opacity-40 disabled:cursor-not-allowed" title="Next">
            <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 6l6 6-6 6" /></svg>
          </button>
          <button onClick={zoomIn} className="hover:bg-fuchsia-100/60 active:bg-fuchsia-200 focus:ring-2 focus:ring-fuchsia-400 transition-all duration-150 rounded-full p-2 text-blue-700 disabled:opacity-40 disabled:cursor-not-allowed" title="Zoom In">
            <svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 9v6m3-3H9" /></svg>
          </button>
          {/* Quiz Icon Button */}
          <button
            onClick={onQuizClick}
            className={`ml-4 rounded-full p-2 text-white font-bold transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-green-300 shadow-lg ${completed ? 'bg-gradient-to-r from-green-400 to-lime-400 animate-pulse-glow hover:scale-110' : 'bg-gray-300/60 cursor-not-allowed opacity-60'}`}
